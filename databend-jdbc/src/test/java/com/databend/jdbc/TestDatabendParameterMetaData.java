    ...
    public void testGetParameterMetaData() throws SQLException {
        try (Connection conn = createConnection();
             ...
             PreparedStatement inputPs = conn.prepareStatement(
                     "insert into non_existing_table ('col2 String, col3 Int8, col1 VARIANT') values (?, ?, ?)");
            ...
            for (PreparedStatement ps : new PreparedStatement[]{inputPs, sqlPs}) {
                ...
                Assert.assertEquals(ps.getParameterMetaData().getParameterTypeName(3), DatabendDataType.VARIANT.name().toLowerCase());
            }
        }
    }
}
